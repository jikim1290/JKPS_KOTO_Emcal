...
Figure~\ref{fig:angle_reco_dep_gr} (a) shows the angular resolution as a function of the incident photon energy ($E_{\gamma}$) at $\theta=$~25$^{\circ}$. Angular resolutions are fitted with a function of $p_{0} \oplus p_{1}/\sqrt{E_{\gamma}(GeV)}$, where $p_{0}$ and $p_{1}$ represent an energy independent and energy dependent contribution, respectively. $p_{0}$ and $p_{1}$ are consistent for $\theta>$~10$^{\circ}$ and estimated to be 0.29$^{\circ}$ and 1.24$^{\circ}$, respectively. For small $\theta$, The values are varying up to $\pm$~0.15$^{\circ}$ as shown in Fig.~\ref{fig:angle_reco_dep_gr} (b).

...
\includegraphics[width=0.48\textwidth]{figures/Fig7_eres.pdf}
}
...
We tune the hyperparameter of the $\XGB$ to reconstruct the incidence angle with the best angular resolution, and study the angular resolution in terms of the detector configuration. We find that 15-mm-wide strips provide the best angular resolution, which can be expressed as 0.45~$\oplus~$1.21$/\sqrt{E_\gamma}$ for different incident angles. We also conclude that using 24 front layers gives the angular resolution which is comparable with the angular resolution we get from the full layer.
